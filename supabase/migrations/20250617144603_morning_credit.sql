...
-- Create custom types (with proper existence checks)
DO $$ BEGIN
...
DROP POLICY IF EXISTS "All authenticated users can read sales" ON sales;
DROP POLICY IF EXISTS "All authenticated users can insert sales" ON sales;

DROP POLICY IF EXISTS "All authenticated users can read credits" ON credits;
DROP POLICY IF EXISTS "All authenticated users can insert credits" ON credits;
DROP POLICY IF EXISTS "Managers and admins can update credits" ON credits;

DROP POLICY IF EXISTS "All authenticated users can read stock adjustments" ON stock_adjustments;
DROP POLICY IF EXISTS "All authenticated users can insert stock adjustments" ON stock_adjustments;

DROP POLICY IF EXISTS "All authenticated users can read daily reports" ON daily_reports;
DROP POLICY IF EXISTS "All authenticated users can insert daily reports" ON daily_reports;
...
BEGIN
  ...
  BEGIN
    ...
    IF user_phone IS NOT NULL THEN
      ...
      VALUES (new.id, user_phone, user_full_name, user_role);
    END IF;
...
-- Trigger for new user creation (drop first to avoid conflicts)
